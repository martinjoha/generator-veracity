...
Before starting this application make sure you have created an application on developer.veracity.com and have received an encrypted email that consists of necessary information to be able to use the application properly.
...
The files mostly concerned with authentication in this application are inside the `init` folder, and `setupAuthentication.js` and `setupDatafabric.js` handles the authentication process for the Services API and Data Fabric API respectively. They are both relatively similar, though you have to use a different scope for obtaining the tokens. 
...
After a user has completed the authentication process, passport issues two tokens, an **access token**, and a **refresh token**. The access token is the one to be attached in the request header when accessing the Veracity APIs, and the refresh token is used to gain a new access token if it has expired. 
...
A user has the get the resourceID of the container:
...
Note that the access token is note the same as the one used for user authentication, but the one obtained when using the data fabric scope.
...
Using one of the accessId's obtainer from this API call, a user now has the required information about the resource to get a SAS-token from Veracity, using the resourceId and an accessId:

```javascript
const response = await request({
	method: "PUT",
	url: `https://api.veracity.com/veracity/datafabric/data/api/1/resources/${req.headers.resourceid}/accesses/${accessId}/key`,
	headers: {
		"Ocp-Apim-Subscription-Key": config.apiKeys.dataFabricApi,
		"Authorization": "Bearer " + req.user.tokens.data.access_token
	}
...
To be able to communicate with Azure Storage, a user needs the SAS-token and the Host URL for the specific storage. The Host URL can be found in the sasUri value obtained after the last API call to the Data API. The Host URL is the part of the URI before `/{containerName}`.
